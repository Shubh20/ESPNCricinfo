...
for i in list1:
    s=str(i)
    spark_df1=spark.read.orc("s3a://projectespn/ODI/"+s+"/*/*.orc")
...
df.rename(columns={"Bat1":"Runs","Unnamed: 9":"ODI_id","Start Date":"Match_Date","Ct":"Catches","St":"Stumpings"},inplace=True)
...
df['ODI_id']=df['ODI_id'].str.replace("ODI # ","")
...
df['ODI_id']=pd.to_numeric(df['ODI_id'])#.astype(int)
df['Player_id']=df['Player_id'].astype(int)
df['Name']=df['Name'].astype(str)
df['Country']=df['Country'].astype(str)  

spark_df=sqlContext.createDataFrame(df)
spark_df.coalesce(1).write.mode("append").option("header","true").format("orc").save("s3a://cleaneddataespn/ODI/")
spark_df.coalesce(1).write.mode("append").option("header","true").format("csv").save("s3a://cleaneddataespn/ODI/")
spark_df.show()
