...
request_link3=';class=2'
for request_no in[1,2,3,4,5,6,7,8,9,20,25,29,40,15,30,32,27,17,26,11]:
    ...
    link3='.html?class=2;template=results;type=allround;view=match'

    link4='.html?class=2;template=results;type=allround;view=innings'
...
#fd_ind_test['over2']=inn2
    ...
    ts=time.time()
    fd_ind_test['Wkts']=fd_ind_test['Wkts'].str.replace("-","")
    fd_ind_test['Conc']=fd_ind_test['Conc'].str.replace("-","")
    fd_ind_test['Ct']=fd_ind_test['Ct'].astype(str).str.replace("-","")
    fd_ind_test['St']=fd_ind_test['St'].astype(str).str.replace("-","")
    ...
    spark_df.coalesce(1).write.mode("append").option("header","true").format("orc").save("s3a://projectespn/ODI/"+team_name+"/"+time_stamp+"/")
